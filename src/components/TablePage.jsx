...
function TablePage() {
  ...
  useEffect(() => {
    setIsLoading(true);
    axios
      .get("http://localhost:3001/users/getAllEntries")
  ...
  const employerList = data
    .filter(
      (i) =>
        i.Employer.toLowerCase().includes(search) ||
        i.Job_Title.toLowerCase().includes(search) ||
        i.University_Name.toLowerCase().includes(search)
    )
    .slice(
      (currentEmployerPage - 1) * 10 + 1,
      (currentEmployerPage - 1) * 10 + 11
    )
    .map((obj) => (
      <li key={obj.id}>
        <Card
  ...
  return (
    <div className="Main">
      ...
      <form className="input">
        ...
        />
        <button onClick={(e) => addToTable(e)} type="submit">
          Add{" "}
        </button>
      </form>
      ...
      <div className="bodySeparation">
        <div className="tablebody1">
          {calculatePagesEmployer().map((i) => {
            return (
              <span
                onClick={() => setCurrentEmployerPage(i)}
